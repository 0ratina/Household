...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    if (!hasTouchedAvatar) {
      ...
      if (a && AVATARS.includes(a as AvatarEmoji))
        setAvatarId(a as AvatarEmoji);
    }
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async ({
      ...
      if (!profileId)
        throw new Error("Kunde inte hitta din profil för valt hushåll.");
      ...
      if (householdId) {
        ...
        if (!allowed)
          throw new Error(
            "Den här avataren är redan vald av någon i hushållet.",
          );
      }
  ...
  const onSave = () => {
    ...
    if (!selectedProfile) {
      alert(
        "Kunde inte hitta din profil för valt hushåll. Öppna hushållet och försök igen.",
      );
      return;
    }
  ...
  if (uid && myProfiles.length === 0) {
    return (
      ...
      >
        ...
        >
          Du har ingen profil ännu. Gå till ett hushåll och skapa/gå med för att
          kunna ändra avatar och namn.
        </Text>
        <TouchableOpacity
